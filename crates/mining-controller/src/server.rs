//! Small synchronous authenticated HTTP server for the mining worker.

use crate::control::MiningControl;
use simchain_common::control_api::{ApiError, ErrorCode};
use simchain_common::internal_api::{
    LeaseReleaseRequest, LeaseRenewRequest, LeaseRequest, SetMiningPolicyRequest, SetStateRequest,
    INTERNAL_API_PREFIX,
};
use std::io::Read;
...
pub fn spawn(
    ...
    let server = Server::http(listen_addr)
        .map_err(|error| anyhow::anyhow!("bind mining control server: {error}"))?;
    tracing::info!(%listen_addr, "mining internal control API listening");
    Ok(std::thread::spawn(move || {
        for request in server.incoming_requests() {
            if let Err(error) = handle_request(request, &token, &control) {
                tracing::warn!("mining control request failed: {error}");
            }
        }
    }))
}
...
fn handle_request(
    ...
    let result = match (method, path.as_str()) {
        ...
        (Method::Put, path) if path == format!("{INTERNAL_API_PREFIX}/config") => {
            match read_json::<SetMiningPolicyRequest>(&mut request) {
