//! Authenticated private HTTP API for the namespace-local network agent.

use crate::control::NetworkControl;
use simchain_common::control_api::{ApiError, ErrorCode};
use simchain_common::internal_api::{
    LeaseRenewRequest, NetworkLeaseReleaseRequest, NetworkLeaseRequest, INTERNAL_API_PREFIX,
};
use std::io::Read;
...
pub fn run(
    ...
    token: String,
    control: Arc<NetworkControl>,
) -> anyhow::Result<()> {
    let server = Server::http(listen_addr)
        .map_err(|error| anyhow::anyhow!("bind network-agent server: {error}"))?;
    tracing::info!(%listen_addr, "network-agent internal API listening");
    for request in server.incoming_requests() {
        if let Err(error) = handle_request(request, &token, &control) {
            tracing::warn!("network-agent request failed: {error}");
        }
    }
    Ok(())
}
...
fn handle_request(
    ...
    match (method, path.as_str()) {
        ...
        (Method::Post, path) if path == format!("{INTERNAL_API_PREFIX}/leases") => {
            match read_json::<NetworkLeaseRequest>(&mut request) {
                Ok(body) => domain_response(request, control.acquire(body)),
                Err(error) => body_error(request, error),
            }
        }
        (Method::Post, path) if lease_path(path, "/renew").is_some() => {
            ...
            match read_json::<LeaseRenewRequest>(&mut request) {
                Ok(body) => domain_response(request, control.renew(&lease_id, body)),
                Err(error) => body_error(request, error),
            }
        }
        (Method::Delete, path) if lease_path(path, "").is_some() => {
            let lease_id = lease_path(path, "").expect("checked path").to_string();
            match read_json::<NetworkLeaseReleaseRequest>(&mut request) {
                Ok(body) => domain_response(request, control.release(&lease_id, body)),
                Err(error) => body_error(request, error),
            }
        }
        _ => respond_error(
...
fn domain_response<T: serde::Serialize>(
    request: Request,
...
fn body_error(request: Request, error: anyhow::Error) -> anyhow::Result<()> {
    respond_error(
        request,
        ApiError::new(
...
mod tests {
    ...
    fn lease_route_parser_is_strict() {
        assert_eq!(
            lease_path("/internal/v1/leases/a/renew", "/renew"),
            Some("a")
        );
        assert_eq!(lease_path("/internal/v1/leases/a", ""), Some("a"));
