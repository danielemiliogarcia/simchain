...
`scenario`, and `panel` profiles stay separate because they are disruptive,
on-demand, or Docker-socket-backed helpers; including them in `all-tools` would run
them during an ordinary startup. To stop and remove containers from every profile,
