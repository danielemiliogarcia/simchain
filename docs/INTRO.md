...
Fills blocks so they are not empty. By default (raw engine) it can run in DATA/HYBRID mode — OP_RETURN data txs of varied sizes that fill blocks at near-zero node cost, kept `SPAM_FILL_BLOCK_RATIO` blocks deep — or in OUTPUT mode, spamming `SPAM_FIXED_TXS_PER_BLOCK` burn-output txs per block. Outputs are paid to burn addresses so no wallet fills with dust. In DATA/HYBRID mode it also maintains a standing pool of `SPAM_FLOOR_POOL_TXS` standalone ~110-vB floor-priced fills, so blocks pack ~100% full and the `SPAM_FEE` price floor is **airtight**: a below-floor tx waits in the mempool until it outbids the floor, like mainnet under congestion. See SETTINGS.md "Spammer". It splits funds into independent branch UTXOs so the 25-tx unconfirmed-chain mempool limit does not cap production; funding and expansion progress between cycles without blocking healthy branches. Live setting changes preserve this engine state, while startup, snapshots, and reorgs retain the full reconciliation path. If you spam many transactions, some may stay in the mempool and join the next batch, tune the settings to achieve the scenario you need, or disable with `ENABLE_SPAM=false`. With `ENABLE_SPAM_REPLACES=true` every spam tx signals RBF and a few per batch get fee-bumped, so the mempool carries real BIP125 replacements (see SETTINGS.md).
